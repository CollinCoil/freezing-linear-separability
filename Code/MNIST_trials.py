...
from create_network import create_fc_network  # Import from create_network.py
...
def load_mnist():
    ...
    # Normalize to [0,1] and flatten images
...
def evaluate_width(layer_base_widths, frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    ...
    for layer_base_width in layer_base_widths:
        for frozen in frozen_options:
            ...
            for trial in range(num_trials):
                ...
                with open("Results/mnist_width.csv", "a") as f:
                    result_df.to_csv(f, header=f.tell() == 0, index=False)

# Example usage
layer_base_widths = [16, 32, 64, 128, 256, 512]
...
def evaluate_depth(num_reservoir_layers, frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    (x_train, y_train), (x_test, y_test) = load_mnist()
    ...
    for num_layers in num_reservoir_layers:
        for frozen in frozen_options:
            ...
            for trial in range(num_trials):
                ...
                # Append the result to the CSV file
                with open("Results/mnist_depth.csv", "a") as f:
                    result_df.to_csv(f, header=f.tell() == 0, index=False)


# Example usage - layers must be run in reverse order for CSV headers to be correct
...
def evaluate_reservoir_scaling(reservoir_layer_scaling_factors, frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    (x_train, y_train), (x_test, y_test) = load_mnist()
    ...
    for scaling_factor in reservoir_layer_scaling_factors:
        for frozen in frozen_options:
            ...
            for trial in range(num_trials):
                ...
                # Append the result to the CSV file
                with open("Results/mnist_scaling.csv", "a") as f:
                    result_df.to_csv(f, header=f.tell() == 0, index=False)
...
def evaluate_frozen_position(positions, frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    (x_train, y_train), (x_test, y_test) = load_mnist()
    ...
    for position in positions:
        for frozen in frozen_options:
            ...
            for trial in range(num_trials):
                ...
                # Append the result to the CSV file
                with open("Results/mnist_position.csv", "a") as f:
                    result_df.to_csv(f, header=f.tell() == 0, index=False)
...
def evaluate_regularization(regularization_options, frozen_options, num_trials=25, epochs=50, batch_size=64):
    (x_train, y_train), (x_test, y_test) = load_mnist()
    results = []
    ...
    for reg_name, reg_params in regularization_options:
        for frozen in frozen_options:
            for trial in range(num_trials):
                ...
                result_df = pd.DataFrame([result_dict])
                with open("Results/mnist_regularization.csv", "a") as f:
                    result_df.to_csv(f, header=f.tell() == 0, index=False)
...
def evaluate_frozen_states(frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    (x_train, y_train), (x_test, y_test) = load_mnist()
    ...
    for frozen in frozen_options:
        ...
        for trial in range(num_trials):
            ...
            # Append the result to the CSV file
            with open("Results/mnist_frozen_states.csv", "a") as f:
                result_df.to_csv(f, header=f.tell() == 0, index=False)
...
def evaluate_training_progress(frozen_options, num_trials=25, epochs=50, batch_size=64):
    # Load data
    (x_train, y_train), (x_test, y_test) = load_mnist()
    ...
    for frozen in frozen_options:
        for trial in range(num_trials):
            ...
            with open("Results/mnist_training_separability.csv", "a") as f:
